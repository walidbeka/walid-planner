...
function generateInvoice(paymentId) {
    ...
    if (p.type === 'installments' && p.installments && p.installments.length) {
        installmentsTable = `<h3 style="margin:20px 0 8px;color:#333">تفاصيل الدفعات</h3>
        <table style="width:100%;border-collapse:collapse;font-size:13px"><thead><tr style="background:#f0f4ff;text-align:right"><th style="padding:8px;border:1px solid #ddd">الدفعة</th><th style="padding:8px;border:1px solid #ddd">المبلغ</th><th style="padding:8px;border:1px solid #ddd">الحالة</th></tr></thead><tbody>` +
            p.installments.map((inst, i) => `<tr><td style="padding:8px;border:1px solid #ddd">${inst.label || 'دفعة ' + (i+1)}</td><td style="padding:8px;border:1px solid #ddd">${parseFloat(inst.amount).toLocaleString('ar-EG')} ج.م</td><td style="padding:8px;border:1px solid #ddd;color:${inst.paid ? '#2e7d32' : '#dc3545'}">${inst.paid ? 'مدفوعة ✓' : 'لم تُدفع ✗'}</td></tr>`).join('') +
            '</tbody></table>';
    }
    const invoiceHTML = `<div style="max-width:700px;margin:0 auto;padding:30px;font-family:Arial,sans-serif;direction:rtl;color:#333">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid #1877f2;padding-bottom:16px;margin-bottom:24px">
        <div><h1 style="margin:0;font-size:28px;color:#1877f2">فاتورة</h1><p style="margin:2px 0;font-size:12px;color:#666">رقم: ${p.id.slice(0,8).toUpperCase()}</p></div>
        <div style="text-align:left"><p style="margin:2px 0;font-size:13px"><strong>Walid Mohammed</strong></p><p style="margin:2px 0;font-size:12px;color:#666">+201274647071</p><p style="margin:2px 0;font-size:12px;color:#666">الهرم — مصر</p><p style="margin:2px 0;font-size:12px;color:#666">بتاريخ: ${today}</p></div>
    </div>
    <div style="background:#f8f9fa;padding:16px;border-radius:8px;margin-bottom:20px">
        <h3 style="margin:0 0 8px;color:#1877f2;font-size:14px">بيانات العميل</h3>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:13px">
            <div style="padding:6px 0"><strong>العميل:</strong> ${p.client || '—'}</div>
            <div style="padding:6px 0"><strong>التليفون:</strong> ${p.phone || '—'}</div>
            <div style="padding:6px 0"><strong>الخدمة:</strong> ${p.service || '—'}</div>
            <div style="padding:6px 0"><strong>المدة:</strong> ${p.duration || '—'}</div>
            <div style="padding:6px 0"><strong>تاريخ البدء:</strong> ${p.date ? new Date(p.date + 'T00:00:00').toLocaleDateString('ar-EG') : '—'}</div>
            <div style="padding:6px 0"><strong>طريقة الدفع:</strong> ${p.type === 'once' ? 'دفعة واحدة' : 'دفعات'}</div>
        </div>
    </div>
    <table style="width:100%;border-collapse:collapse;margin:12px 0;font-size:13px"><thead><tr style="text-align:right"><th style="padding:10px;border:1px solid #ddd;background:#f0f4ff;font-weight:600">البند</th><th style="padding:10px;border:1px solid #ddd;background:#f0f4ff;font-weight:600">المبلغ</th></tr></thead><tbody><tr><td style="padding:10px;border:1px solid #ddd">${p.service || 'خدمة'}</td><td style="padding:10px;border:1px solid #ddd;font-weight:600">${total.toLocaleString('ar-EG')} ج.م</td></tr></tbody></table>
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;font-size:13px;margin-top:12px">
        <div style="padding:6px 0"><strong>المبلغ الكلي:</strong> ${total.toLocaleString('ar-EG')} ج.م</div>
        <div style="padding:6px 0"><strong>المدفوع:</strong> <span style="color:#2e7d32">${paid.toLocaleString('ar-EG')} ج.م</span></div>
        <div style="padding:6px 0"><strong>المتبقي:</strong> <span style="color:#dc3545">${unpaid.toLocaleString('ar-EG')} ج.م</span></div>
    </div>
    ${installmentsTable}
    ${p.notes ? '<div style="margin-top:12px;padding:12px;background:#fff8e1;border-radius:8px;font-size:13px"><strong>ملاحظات:</strong> ' + p.notes + '</div>' : ''}
    <div style="font-size:18px;font-weight:bold;color:#1877f2;margin-top:16px;text-align:left;padding:12px;background:#f0f4ff;border-radius:8px">المبلغ المستحق: ${unpaid.toLocaleString('ar-EG')} ج.م</div>
    <div style="text-align:center;margin-top:30px;padding-top:16px;border-top:1px solid #eee;font-size:11px;color:#999"><p>Walid Mohammed — +201274647071 — الهرم — مصر</p></div>
    </div>`;
    document.getElementById('print-content').innerHTML = invoiceHTML;
    document.getElementById('print-modal').style.display = 'flex';
}
